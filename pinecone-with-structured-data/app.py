...
class QuizzSearchApp:
    ...
    def _handle_chapter_questions(self, message):
        ...
        try:
            ...
            pinecone_results = self.search_pinecone(chapter_title, 10)
    ...
    def _handle_section_questions(self, message):
        ...
        try:
            ...
            pinecone_results = self.search_pinecone(heading_title, 5)
            matched_results = [
                result for result in pinecone_results 
                if heading_title in [k.lower() for k in result['metadata'].get('keywords', [])] or
                heading_title in result['metadata'].get('heading_title', '').lower() or
                heading_title in result['metadata'].get('subheading_title', '').lower() or
                heading_title in result['metadata'].get('subsubheading_title', '').lower()
            ]          
            results_to_use = matched_results if matched_results else pinecone_results
            print(results_to_use)
            all_questions = self.openai_handler.generate_questions(results_to_use, num_questions)
